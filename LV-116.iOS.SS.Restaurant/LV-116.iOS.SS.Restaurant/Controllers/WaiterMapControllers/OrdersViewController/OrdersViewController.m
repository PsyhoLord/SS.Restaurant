    ...
    [MapDataProvider loadMapDataWithBlock:^(MapModel *mapModel, NSError *error) {
        ...
        if ( error ) {
            ...
            dispatch_async( dispatch_get_main_queue(), ^{
                ...
                // Set initial values for _flagsContainer (if @NO - rows doesn't appear at start; if @YES - rows appear at start).
...
- (NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if( _flagsContainer ){
        if ( [[_flagsContainer objectAtIndex: section ] boolValue] ) {
            ...
            NSInteger numberOfOrders = ((WaiterTableModel*)_waiterMapModel.arrayOfTableModel[section]).arrayOfOrders.count +1;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    sectionButton.frame = CGRectMake(0, 0, 130, 30); //set some large width to ur title
...
// Add editing style for cell. Add cell has not any editing style
